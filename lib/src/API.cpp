/**
  ...
  * @version v1.0
  * @date    08-12-25
  * @brief   Implementation of Graphiti API functionality.
  *
  * @details
  * Implements Graphiti_API methods including command building, response parsing,
  * checksum handling and IO helpers that drive the Graphiti device.
  * Contains response loop and event queue management.
  *
  ******************************************************************************
  * @attention
 ...
 * @brief Construct a new Graphiti_API object without connection.
 */
 ...
 * @param connection Pointer to GraphitiConnection for read calls.
 */
...
/**
 * @function ~Graphiti_API
 * @brief Destroy the Graphiti_API object and stop response thread.
 */
Graphiti_API::~Graphiti_API() {
    stopResponseThread();
}
...
/**
 * @function initMaps
 * @brief Initialize internal lookup maps (responses, keys, gestures).
 */
 ...
 * @brief Set the active connection used by the API.
 ...
 * @brief Start the background response handling thread.
 */
...
/**
 * @function runResponseLoop
 * @brief Background loop that continuously reads responses.
 */
 ...
 * @brief Stop the background response thread and join it.
 */
 ...
 * @brief Read a single response frame and dispatch to the appropriate handler.
 */
...
/**
 * @function setUpResponseMap
 * @brief Initialize the response opcode → handler dispatch map.
 */
 ...
 * @brief Reads bytes from the device and converts them into an ASCII string.
 * @param data Reference to vector to store bytes.
 * @param length Number of bytes to read.
 * @return std::optional<std::string> ASCII string if successful, nullopt otherwise.
 */
...
/**
 * @function checkEndByte
 * @brief Validates checksum and sends ACK/NACK based on result.
 * @param data Data vector to check.
 * @param startByte Start byte for checksum.
 * @return bool True if checksum passes, false otherwise.
 */
 ...
 * @brief Reads command response, verifies checksum, and queues the response string.
 * @param startByte Start byte for command.
 * @param funtion_name Name of the function.
 ...
 * @brief Get next available output event or null if none exist.
 * @return std::optional<std::string> Next output event if available.
 */
...
/**
 * @function readBytes
 * @brief Reads raw bytes from device into a buffer.
 * @param data Reference to vector to store bytes.
 * @param length Number of bytes to read.
 * @return bool True if all bytes read, false otherwise.
 */
...
/**
 * @function readStatus
 * @brief Reads display/status response and pushes it into event queue.
 * @param startByte Start byte for status.
 * @param returnName Name of the return value.
 * @param length Number of bytes to read.
 */
 ...
 * @brief Retrieves next display status event packet if available.
 * @return std::optional<std::vector<uint8_t>> Next display status event if available.
 */
...
/**
 * @function readKeyEventStatus
 * @brief Reads device key-event status (enabled/disabled).
 */
 ...
 * @brief Initializes mapping table to decode hardware key codes.
 */
 ...
 * @brief Reads key press event and converts raw codes to readable key names.
 */
...
/**
 * @function readKeyEventPlus
 * @brief Reads key press event and converts raw codes to readable key names.
 */
void Graphiti_API::readKeyEventPlus() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (bytes.empty()) {
            std::cerr << "readKeyEventPlus timed out" << std::endl;
            return;
        }
    ...
    if (!checkEndByte(data, 0x33)) {
        std::cerr << "readKeyEventPlus checksum failed" << std::endl;
        return;
    }

    // Combine first 3 bytes into 24-bit key field
    uint64_t keyBits = 0;

    keyBits |= static_cast<uint64_t>(data[0]);        // Byte 3
    keyBits |= static_cast<uint64_t>(data[1]) << 8;   // Byte 4
    keyBits |= static_cast<uint64_t>(data[2]) << 16;  // Byte 5

    std::set<std::string> keyEvent;

    // Decode standard keys
    for (const auto& [mask, name] : keyMap_plus) {
        if (keyBits & mask) {
            keyEvent.insert(name);
        }
    }

    for (int byteIndex = 3; byteIndex < 8; byteIndex++) {
        ...
        for (int bit = 0; bit < 8; bit++) {
            ...
            if (value & (1 << bit)) {
                ...
                int crNumber = ((byteIndex - 3) * 8) + bit + 1;

                keyEvent.insert("CR-" + std::to_string(crNumber));
            }
        }
    }

    if(data[8] == 0x02) keyEvent.insert("Short pressed");
    else if(data[8] == 0x04) keyEvent.insert("Long pressed");
 ...
 * @brief Retrieves next key event from the queue if available.
 * @return std::optional<std::set<std::string>> Next key event if available.
 */
...
/**
 * @function printKeyEvent
 * @brief Helper to pretty-print a set of key strings to stderr.
 * @param keyEvent Set of key strings.
 */
...
/**
 * @function readTouchEventStatus
 * @brief Read touch event status byte from device and log enabled/disabled.
 */
...
/**
 * @function setUpGestureMap
 * @brief Initialize the gesture code → string mapping.
 */
...
/**
 * @function readGestureEvent
 * @brief Read a gesture event code from the device and push to queue.
 */
 ...
 * @brief Pop the next gesture event from the internal queue, if any.
 * @return std::optional<std::string> Next gesture event if available.
 */
...
/**
 * @function readDrawEvent
 * @brief Read a draw event from the device and convert into DrawEvent structure.
 */
 ...
 * @brief Pop next DrawEvent from queue if available.
 * @return std::optional<Graphiti_API::DrawEvent> Next draw event if available.
 */
...
/**
 * @function setUpCommonResponseMap
 * @brief Initialize the common response code -> string map.
 */
...
/**
 * @function readCommonResponse
 * @brief Read a one-byte common response and print its text.
 */
 ...
 * @brief Calculate checksum for a vector of bytes.
 * @param data Vector of bytes to calculate checksum for.
 * @return std::vector<uint8_t> Checksum bytes.
 */
...
/**
 * @function Graphiti_IO_Write
 * @brief Write to Graphiti adding the checksum and Header Byte.
...
/**
 * @function startOfFrameCheck
 * @brief Insert SOF bytes and start byte as needed.
 * @param bytes Data vector.
 * @param startByte Start byte.
 * @return std::vector<uint8_t> Modified data vector.
 */
...
/**
 * @function sendACK
 * @brief Send an ACK command to the device.
 */
...
/**
 * @function sendNACK
 * @brief Send a NACK command to the device.
 */
 ...
 * @brief Request the software version from the device.
 */
 ...
 * @brief Request the hardware version from the device.
 */
 ...
 * @brief Request the device serial number.
 */
 ...
 * @brief Request the device battery status.
 */
 ...
 * @brief Request display resolution information from the device.
 */
 ...
 * @brief Request current device orientation.
 */
 ...
 * @brief Request height-related information from the device.
 */
 ...
 * @brief Request the device name string.
 */
 ...
 * @brief Send a full screen buffer to the device to update the display.
 * @param screen Raw display bytes to send.
 */
...
/**
 * @function setDisplay
 * @brief Send command to set display mode on the device.
 */
 ...
 * @brief Clear the device display.
 */
 ...
 * @brief Update one pixel at the specified row/column with height and blink rate.
 * @param row Row index.
 ...
 * @brief Update an entire row; rowData contains the row bytes.
 * @param row Row index.
 * @param rowData Row data bytes.
 */
 ...
 * @brief Update an entire column; columnData contains the column bytes.
 * @param column Column index.
 * @param columnData Column data bytes.
 */
 ...
 * @brief Request position/status for all pixels on the device.
 */
 ...
 * @brief Request position/status for a single pixel.
 * @param row Row index.
 * @param column Column index.
 */
 ...
 * @brief Request position/status for a single row.
 * @param row Row index.
 */
 ...
 * @brief Request position/status for a single column.
 * @param column Column index.
 */
 ...
 * @brief Display a message on the device with selection flags and blink rate.
 * @param message Text to show.
 * @param selectFlags Per-character flag bytes.
 * @param blinkRate Blink speed.
 */
void Graphiti_API::showMessage(std::string message, std::vector<uint8_t> selectFlags, uint8_t blinkRate) {
    ...
    std::cout << selectFlags.size() << std::endl;

    for (size_t i = 0; i < messageBytes.size(); i++)
    {
        data[i*2] = messageBytes[i];
    }
    for (size_t i = 0; i < selectFlags.size(); i++)
    {
        data[i*2 + 1] = selectFlags[i];
    }
    
    data.insert(data.end(), static_cast<uint8_t>(blinkRate));
    Graphiti_IO_Write(startOfFrameCheck(data, 0x24));
}

/**
 * @function showMessageOnBraille
 * @brief Display a message on the Braille display of the device.
 * @param message Braille data.
 * @param count max number of Braille cells
 */
void Graphiti_API::showMessageOnBraille(const std::vector<uint8_t> message, uint8_t count) {

    std::vector<uint8_t> data(count + 1, 0);

    std::cout << message.size() << std::endl;

    data[0] = count;

    for (size_t i = 0; i < count && i < message.size(); i++) {
        data[i + 1] = message[i];
    }

    Graphiti_IO_Write(startOfFrameCheck(data, 0x80));
}

/**
 * @function setCursor
 * @brief Configure the cursor position, height, length and blink rate.
 * @param row Row index.
 * @param column Column index.
 * @param height Height value.
 ...
 * @param blinkRate Blink rate value.
 */
void Graphiti_API::setCursor(uint8_t row, uint8_t column, uint8_t height, uint8_t length, uint8_t blinkRate) {
    Graphiti_IO_Write(startOfFrameCheck({ 
        row, column, height, length, blinkRate 
    }, 0x25));
}
 ...
 * @brief Start an interruptible image transfer to the device.
...
/**
 * @function sendImage
 * @brief Internal helper to perform the actual image transfer.
 * @param commandByte Protocol command (interruptible/blocking).
 * @param name Filename.
 ...
 * @brief Enable or disable key event notifications from the device.
 ...
 * @brief Enable or disable touch event notifications from the device.
 * @param enabled True to enable, false to disable.
 */
 ...
 * @brief Request the last touch point status from the device.
 */
 ...
 * @brief Control the vibrator motor with frequency range, duty cycle and duration.
 * @param frequencyRange Frequency range.
 * @param dutyCycle Duty cycle.
 * @param duration Duration in ms.
 */
void Graphiti_API::vibratorControlCommand(uint8_t frequencyRange, uint8_t dutyCycle, uint16_t duration) {
    Graphiti_IO_Write(startOfFrameCheck({ frequencyRange, dutyCycle, 
 ...
 * @brief Request the device's current date and time.
 */
 ...
 * @brief Set the device's date and time.
 * @param day Day value.
 ...
 */
void Graphiti_API::setDateAndTime(uint8_t day, uint8_t month, uint16_t year, uint8_t hour, uint8_t minute, uint8_t second) {
    Graphiti_IO_Write(startOfFrameCheck({ 
        day, month,
...
/**
 * @function debugByte
 * @brief Print a single byte in hex when debugging is enabled.
...
/**
 * @function printVectorHex
 * @brief Print a vector of bytes in hex format to stderr (debugging helper).
