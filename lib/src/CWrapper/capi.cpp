...
extern "C" {
    ...
    */
     ...
     * @brief Display a message on the Braille display of device.
     * @param handle Graphiti handle.
     * @param message Message data.
     * @param count max number of Braille cells.
     */
    void graphiti_showMessageOnBraille(GraphitiHandle* handle, const uint8_t* message, int msgLength, int count) {
        std::vector<uint8_t> msg(message, message + msgLength);
        handle->api.showMessageOnBraille(msg, count);
    }
    /**
