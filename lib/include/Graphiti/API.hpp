...
public:
     ...
     * @brief 4.3.14 Show Message of the Braille display on the Device
     * @param message char array message up to 40 characters
     * @param count max number of Braille cells (40)
     * 
     * @details The 'Show Message On Braille' command is used to display a message on the 
     * Braille display of the device.
     */
    void showMessageOnBraille(std::vector<uint8_t> message, uint8_t count);
...
private:
    ...
    /**
     * @brief Key Map for Graphiti Plus
     * 
     * Mapping of int to string key name
     */
    std::map<uint64_t, std::string> keyMap_plus;
    ...
    /**
     * @brief 4.4.1 Read Key Event for Graphiti Plus
     * 
     * Responses from enable 'Set Key Event' command.
     * Uses bit masks to get each digit then creates key event.
     * 
     */
    void readKeyEventPlus();
