...
<#
 SYNTAX
 Project helper: run build/config/clean/execute tasks using CMake.

 DESCRIPTION
 Convenience script providing `clean`, `configure`, `build` and run helpers
 for the repository. Intended for local development on Windows (PowerShell).
#>
...
<#
 .SYNOPSIS
 @brief Remove the build directory and its contents
#>
function clean {
...
<#
 .SYNOPSIS
 @brief Configure the CMake build (optionally Debug)
#>
function configure {
    param (
        [switch]$Debug
...
<#
 .SYNOPSIS
 @brief Build the project (optional configuration)
#>
function build {
    param (
        [string]$Config = ""
...
<#
 .SYNOPSIS
 @brief Run the ExtensionTest executable from the build
#>
...
<#
 .SYNOPSIS
 @brief Run the Testing executable from the build
#>
function run_Testing {
    ...
    $exePath = if ($Config) { ".\build\$Config\Testing.exe" } else { ".\build\Debug\Testing.exe" }
