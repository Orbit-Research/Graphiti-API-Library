...
class Graphiti:
    ...
    def _setup_types(self):
        ...
        self._lib.graphiti_showMessageOnBraille.argtypes = [c_void_p, POINTER(c_uint8), c_int, c_int]
        self._lib.graphiti_showMessageOnBraille.restype = None
    ...
    # @function show_message_Braille
    # @brief Render a textual message on the device Braille display.
    # @param self The Graphiti instance.
    # @param message Message data.
    # @param count max number of Braille cells.
    # @return None
    def show_messageOnBraille(self, message: bytes, count: int):
        msg_buf = (c_uint8 * len(message))(*message)
        self._lib.graphiti_showMessageOnBraille(
            self._handle,
            msg_buf,
            len(message),
            count
