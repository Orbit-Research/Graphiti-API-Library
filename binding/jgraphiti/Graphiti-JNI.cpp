...
/**
 * @function Java_binding_jgraphiti_Graphiti_showMessageOnBraille
 * @brief JNI wrapper - display a message on the Braille display of the device.
 * @param env JNI environment pointer.
 * @param obj Java object.
 * @param handle Native handle pointer as long.
 * @param message Java message data.
 * @param count max number of braille cells.
 */
JNIEXPORT void JNICALL Java_binding_jgraphiti_Graphiti_showMessageOnBraille(
    JNIEnv* env, jobject obj, jlong handle,
    jbyteArray message,  jint count
) {
    // Convert jbyteArray → std::vector<uint8_t>
    jbyte* msg = env->GetByteArrayElements(message, nullptr);
    jsize length = env->GetArrayLength(message);
    std::vector<uint8_t> msgVec(msg, msg + length);  // Copies data
    env->ReleaseByteArrayElements(message, msg, JNI_ABORT);

    // 3. Call C++ API
    reinterpret_cast<GraphitiHandle*>(handle)->api.showMessageOnBraille(msgVec, count);
}
