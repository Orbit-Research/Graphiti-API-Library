...
Here, bytes 3 to 42 represent data in Braille format. The input data must first be converted into Braille format and then transmitted to the Braille display using this command.

| Byte 0 | Byte 1 | Byte 2 | Byte 3-42 | Byte 43 |
|--------|--------|----------|----------|----------|
| 0x1B | 0x80 | Length (40) | Braille Data | Checksum |

Bit 0-7 of the each Braille Data Byte displays on Braille cells as per following:
```
0  3
1  4
2  5
...
#### 4.4.2 Key Events for Graphiti
If Key events enabled, on each key press, you will receive the following response without any request from the host.
...
#### 4.4.3 Key Events for Graphiti Plus
If Key events enabled, on each key press, you will receive the following response without any request from the host.

| Byte 0 | Byte 1 | Byte 2-9 | Byte 10 | Byte 11 |
|--------|--------|--------|--------|--------|
| 0x1B | 0x33 | Key values | Key Press Type | Checksum |

The key values sent by the device for different keys are shown in the table below.

| Byte   | Bit 7 | Bit 6 | Bit 5 | Bit 4 | Bit 3 | Bit 2 | Bit 1 | Bit 0 |
|--------|--------|--------|--------|--------|--------|--------|--------|--------|
| Byte 2 | DOT 8  | DOT 7  | DOT 6  | DOT 5  | DOT 4  | DOT 3  | DOT 2  | DOT 1  |
| Byte 3 | ---    | ---    | RIGHT  | LEFT   | DOWN   | UP     | SPACE  | SELECT |
| Byte 4 | ---    | ---    | ---    | ---    | RPF    | RPB    | LPF    | LPB    |
| Byte 5 | CR-8   | CR-7   | CR-6   | CR-5   | CR-4   | CR-3   | CR-2   | CR-1   |
| Byte 6 | CR-16  | CR-15  | CR-14  | CR-13  | CR-12  | CR-11  | CR-10  | CR-9   |
| Byte 7 | CR-24  | CR-23  | CR-22  | CR-21  | CR-20  | CR-19  | CR-18  | CR-17  |
| Byte 8 | CR-32  | CR-31  | CR-30  | CR-29  | CR-28  | CR-27  | CR-26  | CR-25  |
| Byte 9 | CR-40  | CR-39  | CR-38  | CR-37  | CR-36  | CR-35  | CR-34  | CR-33  |

If multiple keys are pressed together, the device will send the bits with the value 1 of the pressed keys. For example, if DOT1 and DOT 4 keys are pressed together, the device will send a Byte2 = 0x09 and other bytes = 0. If SPACE + DOT 5 pressed then, Byte2 = 0x10, Byte3 = 0x02 and other bytes = 0. At the host side, bit level operations can help to decode the exact key press from the multiple key input.

Key Press Type = 0x02 --> Short press  
Key Press Type = 0x04 --> Long press

**Key positions:**

```
     CR-1 CR-2 CR-3 ............................................ CR-39 CR-40
LPB                                                                            RPB

LPF                                                                            RPF

        DOT 7   DOT 3   DOT2   DOT1   Up   DOT 4   DOT 5   DOT 6   DOT8
                             Left   Select   Right
                                     Down
                                    Space
```
...
| Key event | Function |
|-----------|----------|
| DOT 5 + DOT 8 + RPF | Firmware upgrade |
| DOT 5 + DOT 6 + RPF | DFU mode (developer use) |
| DOT 8 + RPF | Hard reset |
| DOT 7 + DOT 8 | Unregister key events from the device |
...
#### 5.3.14 Update Braille Display

This command is used to update the Braille display. It allows the entire Braille display content to be refreshed in a single operation. There is no provision to update an individual cell; the complete display data must be sent each time an update is required.

Here, bytes 4 to 43 represent data in Braille format. The input data must first be converted into Braille format and then transmitted to the Braille display using this command.

| Byte 0 | Byte 1 | Byte 2 | Byte 3 | Byte 4-43 | Byte 44 |
|--------|--------|----------|----------|----------|--------|
| 0x02 | 0x1B | 0x80 | Length (40) | Braille Data | Checksum |
...
#### 5.4.3 Key Events for Graphiti Plus
If Key events enabled, on each key press, you will receive the following response without any request from the host.

| Byte 0 | Byte 1 | Byte 2 | Byte 3-10 | Byte 11 | Byte 12 |
|--------|--------|--------|--------|--------|--------|
| 0x07 | 0x1B | 0x33 | Key values | Key Press Type | Checksum |

The key values sent by the device for different keys are shown in the table below.

| Byte   | Bit 7 | Bit 6 | Bit 5 | Bit 4 | Bit 3 | Bit 2 | Bit 1 | Bit 0 |
|--------|--------|--------|--------|--------|--------|--------|--------|--------|
| Byte 3 | DOT 8  | DOT 7  | DOT 6  | DOT 5  | DOT 4  | DOT 3  | DOT 2  | DOT 1  |
| Byte 4 | ---    | ---    | RIGHT  | LEFT   | DOWN   | UP     | SPACE  | SELECT |
| Byte 5 | ---    | ---    | ---    | ---    | RPF    | RPB    | LPF    | LPB    |
| Byte 6 | CR-8   | CR-7   | CR-6   | CR-5   | CR-4   | CR-3   | CR-2   | CR-1   |
| Byte 7 | CR-16  | CR-15  | CR-14  | CR-13  | CR-12  | CR-11  | CR-10  | CR-9   |
| Byte 8 | CR-24  | CR-23  | CR-22  | CR-21  | CR-20  | CR-19  | CR-18  | CR-17  |
| Byte 9 | CR-32  | CR-31  | CR-30  | CR-29  | CR-28  | CR-27  | CR-26  | CR-25  |
| Byte 10 | CR-40  | CR-39  | CR-38  | CR-37  | CR-36  | CR-35  | CR-34  | CR-33  |

If multiple keys are pressed together, the device will send the bits with the value 1 of the pressed keys. For example, if DOT1 and DOT 4 keys are pressed together, the device will send a Byte3 = 0x09 and other bytes = 0. If SPACE + DOT 5 pressed then, Byte3 = 0x10, Byte4 = 0x02 and other bytes = 0. At the host side, bit level operations can help to decode the exact key press from the multiple key input.
...
| 1.1 | 20th March 2026 | Added VID/PID details for Marble Plus | |
| 1.2 | 12th May 2026 | Added Braille Display command and key events for Marble Plus | |